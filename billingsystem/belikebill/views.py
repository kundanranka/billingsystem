...
def addsupplier(request):
	...
	if login_==True:
		if request.method=='POST':
			...
			supplier['usr']=usr
			cx.prepare("insert into supplier values(:suppliername,:address,:phone,:gst,:usr)")
...
def billing(request):
	...
	if login_==True:
		...
		if request.POST.get('print')==None and request.POST.get('name') != None:
			...
			for i in cx:
				if i[3]>=product['quantity']:
					product['rate']=i[2]
					product['total']=i[2]*product['quantity']
					total+=product['total']
					flag1=1
				flag=1
			if flag==0:
...
def inventory(request):
	...
	if login_==True:
		if request.method=='POST':
			...
			product['usr']=usr
			c.prepare('select suppliername from supplier where username=:usr and suppliername=:sn')
